...
*   Residents can now **sort their block list** — *Curriculum order* (default), *Name (A–Z)*, or *Unfinished first* — from a dropdown above "Board Review Blocks." The choice is **saved per person** on their own device, so everyone can order their list however they like. *(Parked idea #2 — resident side; the matching admin-side sort is coming next.)*
