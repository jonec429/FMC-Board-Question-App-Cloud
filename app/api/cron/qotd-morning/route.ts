  ...
  try {
    ...
    if (existingLog && existingLog.length > 0) {
      ...
      if (lastRunEst.toDateString() === estDate.toDateString()) {
        console.log('[qotd-morning] Already ran successfully today. Exiting early.');
    ...
    if (targetSubs.length === 0) {
      console.log('[qotd-morning] No subscriptions opted in for QOTD morning push.');
      return NextResponse.json({ success: true, message: 'No users opted in.', counts: { total: 0 } });
    }

    const run_id = crypto.randomUUID();
    console.log(`[qotd-morning] Found ${targetSubs.length} opted-in subscription(s). Sending notifications... (Run ID: ${run_id})`);
    ...
    const notifications = targetSubs.map(async (sub) => {
      ...
      try {
        ...
        if ((err as any).statusCode === 404 || (err as any).statusCode === 410) {
          console.log(`[qotd-morning] Subscription expired (${(err as any).statusCode}), deleting: ${sub.endpoint.slice(0, 60)}...`);
          await supabase.from('web_push_subscriptions').delete().eq('endpoint', sub.endpoint);
          expired++;
        } else {
          console.error(`[qotd-morning] Push failed for ${sub.endpoint.slice(0, 60)}...:`, (err as any).statusCode, (err as any).body || (err instanceof Error ? err.message : String(err)));
          failed++;
        }
      }
    });
