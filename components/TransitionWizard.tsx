...
export default function TransitionWizard({ roster, onClose, onRefresh }: TransitionWizardProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-slate-100 dark:border-slate-800 flex flex-col">
        {/* Header */}
        <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur z-10">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center">
              <GraduationCap className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-xl font-black text-slate-800 dark:text-white">Academic Year Transition</h2>
              <p className="text-sm font-bold text-slate-500 dark:text-slate-400">Entering {formatAcademicYear(academicYear)}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="flex border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
          <div className={`flex-1 py-4 text-center font-bold text-sm ${step === 1 ? 'text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-600 dark:border-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>1. Graduate PGY3s</div>
          <div className={`flex-1 py-4 text-center font-bold text-sm ${step === 2 ? 'text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-600 dark:border-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>2. Auto-Advance</div>
          <div className={`flex-1 py-4 text-center font-bold text-sm ${step === 3 ? 'text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-600 dark:border-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>3. Onboard PGY1s</div>
        </div>

        {/* Step 1: Graduate PGY3s */}
        {step === 1 && (
          <div className="p-8 space-y-6">
            <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/50 p-4 rounded-2xl text-blue-800 dark:text-blue-300 text-sm font-medium">
            ...
            {graduatingResidents.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {graduatingResidents.map(r => (
                  <div key={r.email} className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 p-3 rounded-xl flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-emerald-400" />
                    <div>
                      <p className="font-bold text-slate-700 dark:text-white text-sm">{r.name}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{r.email}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                disabled={submitting}
                className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-indigo-200 dark:shadow-none"
              >
        ...
        {step === 2 && (
          <div className="p-8 space-y-6 flex flex-col items-center justify-center text-center py-16">
            <div className="w-20 h-20 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-500 dark:text-emerald-400 rounded-full flex items-center justify-center mb-4">
              <Target className="w-10 h-10" />
            </div>
            <h3 className="text-2xl font-black text-slate-800 dark:text-white">Seamless Auto-Advance</h3>
            ...
            <div className="pt-8">
              <button
                onClick={() => setStep(3)}
                className="px-6 py-3 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold rounded-xl flex items-center gap-2 transition-all shadow-lg border border-transparent dark:border-slate-700"
              >
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="overflow-x-auto border border-slate-200 dark:border-slate-800 rounded-2xl">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800 text-left">
                    <th className="px-4 py-3 font-bold text-slate-500 dark:text-slate-400 w-1/4">First Name</th>
                    <th className="px-4 py-3 font-bold text-slate-500 dark:text-slate-400 w-1/4">Last Name</th>
                    <th className="px-4 py-3 font-bold text-slate-500 dark:text-slate-400 w-1/4">Email</th>
                    <th className="px-4 py-3 font-bold text-slate-500 dark:text-slate-400 w-1/4">Faculty Advisor</th>
                    ...
                    <tr key={i}>
                      <td className="px-2 py-2">
                        <input
                          ...
                          placeholder="First"
                          className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-lg outline-none focus:border-indigo-400 font-medium"
                        />
                      </td>
                      <td className="px-2 py-2">
                        <input
            ...
            <button
              ...
              className="text-indigo-600 dark:text-indigo-400 font-bold flex items-center gap-2 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors text-sm px-2"
            >
            ...
            <div className="flex justify-between items-center pt-8 mt-4 border-t border-slate-100 dark:border-slate-800">
              ...
              <button
                ...
                disabled={submitting}
                className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-emerald-200 dark:shadow-none"
              >
                {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <CheckCircle className="w-5 h-5" />}
                Complete Transition
              </button>
