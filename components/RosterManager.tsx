      ...
      cell: info => {
        ...
        return (
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 rounded-xl flex items-center justify-center font-black shrink-0">
              {member.full_name?.charAt(0) || 'U'}
            </div>
            <div>
              <p className="font-bold text-slate-800 dark:text-white">{member.full_name ? formatLastNameFirst(member.full_name, member.last_name) : 'Incomplete Profile'}</p>
              <p className="text-xs font-medium text-slate-400 dark:text-slate-400">{member.email}</p>
            </div>
          </div>
        );
      },
        ...
        return (
          <div className="flex flex-col gap-1 items-start">
            <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest inline-block ${
              grad ? 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500' : isStaff ? 'bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
            }`}>
              {label}
            </span>
            {member.role === 'admin' && (
              <span className="bg-red-50 dark:bg-red-950/60 text-red-600 dark:text-red-300 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest inline-block mt-1">
                Admin
              </span>
            )}
            {member.advisor && (
              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-400 pl-1 italic mt-1">
      ...
      cell: info => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-bold ${member.has_account ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
            ...
            {!member.has_account && (
              <button
                ...
                className="ml-2 p-1.5 bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/60 transition-colors"
        ...
        return (
          <div className="flex items-center justify-end gap-2">
            ...
            <button 
              ...
              className="p-2 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-lg transition-all"
              title="Remove Person"
            >
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
        <div className="flex items-center gap-4 w-full lg:w-auto">
          <label className="flex items-center gap-2 text-sm font-bold text-slate-600 dark:text-slate-300 cursor-pointer select-none shrink-0 bg-white dark:bg-slate-900 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <input
              type="checkbox"
        ...
        <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto mt-4 lg:mt-0">
          <button
            ...
            className="w-full md:w-auto px-6 py-4 bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-slate-700 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-blue-50 dark:hover:bg-slate-700 active:scale-95 transition-all shadow-sm"
          >
          ...
          <button
            ...
            className="w-full md:w-auto px-6 py-4 bg-indigo-600 text-white rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-indigo-700 active:scale-95 transition-all shadow-xl shadow-indigo-200 dark:shadow-none"
          >
            Year Transition Wizard
          </button>
          <button 
            onClick={() => setShowAddModal(true)}
            className="w-full md:w-auto px-6 py-4 bg-blue-600 text-white rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-blue-700 active:scale-95 transition-all shadow-xl shadow-blue-200 dark:shadow-none"
          >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 dark:border-slate-800">
            <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <h2 className="text-xl font-black text-slate-800 dark:text-white">Add Authorized Person</h2>
              <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 font-bold">Close</button>
            </div>
            <form onSubmit={handleAddPerson} className="p-8 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 outline-none focus:ring-2 focus:ring-blue-600 transition-all font-bold text-slate-800 dark:text-white"
              ...
              <button 
                type="submit" 
                disabled={submitting}
                className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl shadow-blue-100 dark:shadow-none hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
              >
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 dark:border-slate-800">
            <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <h2 className="text-xl font-black text-slate-800 dark:text-white">Edit Authorized Person</h2>
              <button onClick={() => setShowEditModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 font-bold">Close</button>
            </div>
            <form onSubmit={handleEditPerson} className="p-8 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 outline-none text-slate-500 dark:text-slate-400 font-bold cursor-not-allowed"
