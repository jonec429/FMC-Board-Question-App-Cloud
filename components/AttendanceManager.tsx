...
export default function AttendanceManager() {
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-500 pb-20">
      <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 rounded-2xl flex items-center justify-center shrink-0">
            <Database className="w-8 h-8" />
          </div>
          <div>
            <h2 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Attendance Center</h2>
            <p className="text-slate-500 dark:text-slate-400 font-medium max-w-lg">Bulk import monthly New Innovations reports or manually award attendance points to specific residents.</p>
          </div>
        </div>

        {/* Tab Switcher */}
        <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl shrink-0 border border-slate-200/50 dark:border-slate-700/50">
          <button
      ...
      {activeTab === 'bulk' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-black text-slate-800 dark:text-white">NI Data Ingestion</h3>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Academic Year</label>
                  <select
                    value={selectedYear}
                    onChange={(e) => setSelectedYear(Number(e.target.value))}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl px-4 py-3 font-medium outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Target Block</label>
                  <select
              ...
              <div className="mb-6">
                <label className="block w-full p-6 border-2 border-dashed border-blue-200 dark:border-blue-900/60 rounded-2xl flex flex-col items-center justify-center bg-blue-50/50 dark:bg-blue-950/20 hover:bg-blue-50 dark:hover:bg-blue-950/40 transition-colors cursor-pointer group">
                  <input 
                    type="file" 
                  ...
                  <span className="font-bold text-blue-700 dark:text-blue-300">Select an Export File</span>
                  <span className="text-xs text-blue-500 dark:text-blue-400 mt-1 font-medium">Supports CSV, HTML, or raw text exports</span>
                </label>
              </div>

              <div className="flex items-center gap-4 my-6">
                <div className="h-px bg-slate-100 dark:bg-slate-800 flex-1" />
                <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">or paste text</span>
                <div className="h-px bg-slate-100 dark:bg-slate-800 flex-1" />
              </div>
              ...
              <textarea
                ...
                className="w-full h-40 p-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl focus:border-blue-500 focus:ring-0 transition-all font-mono text-xs text-slate-800 dark:text-white placeholder:text-slate-400"
              />
              
              <button
                onClick={handleSmartPaste}
                disabled={!pasteContent.trim() || loading}
                className="w-full mt-6 py-4 bg-blue-600 text-white rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-blue-700 transition-all disabled:opacity-50 shadow-xl shadow-blue-100 dark:shadow-none"
              >
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm min-h-[400px] flex flex-col">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 rounded-xl">
                    <Users className="w-5 h-5" />
                  </div>
                  <h3 className="text-xl font-black text-slate-800 dark:text-white">Matched Credits ({parsedData.filter(p=>p.matched).length})</h3>
                </div>
                ...
                {parsedData.some(p => p.matched) && (
                  <button
                    ...
                    disabled={saving}
                    className="px-6 py-2 bg-emerald-600 text-white rounded-xl font-black text-sm flex items-center gap-2 hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100 dark:shadow-none"
                  >
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-2 scrollbar-hide">
                {parsedData.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-300 dark:text-slate-600">
                    ...
                    <p className="font-bold text-slate-400 dark:text-slate-500">Waiting for data...</p>
                  </div>
                ) : (
                  parsedData.map((person, idx) => (
                    <div 
                      key={idx} 
                      className={`p-4 rounded-2xl border flex items-center justify-between ${person.matched ? 'bg-emerald-50/50 dark:bg-emerald-950/30 border-emerald-100 dark:border-emerald-900/40' : 'bg-red-50/50 dark:bg-red-950/30 border-red-100 dark:border-red-900/40'}`}
                    >
                      <div className="flex flex-col">
                        <span className={`font-bold ${person.matched ? 'text-emerald-900 dark:text-emerald-200' : 'text-red-900 dark:text-red-200'}`}>{person.name}</span>
                        <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase truncate max-w-[200px]">{person.raw}</span>
                      </div>
                      {person.matched ? (
        ...
        <div className="max-w-3xl mx-auto bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-2.5 bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 rounded-2xl">
              <PlusCircle className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-xl font-black text-slate-800 dark:text-white">Manually Award Attendance Points</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Assign customized attendance credits to one, multiple, or all residents at once.</p>
            </div>
          </div>

          {manualSuccessMsg && (
            <div className="mb-6 p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-200 rounded-2xl flex items-center gap-3 font-bold text-sm">
          ...
          <form onSubmit={handleManualSave} className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  <button
                    ...
                    className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg text-xs font-extrabold transition-all"
                  >
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/60 rounded-lg text-xs font-bold transition-all"
                  >
                    + PGY 1
                  </button>
                  <button
                  ...
                  {selectedEmails.length > 0 && (
                    <button
                      ...
                      className="px-2.5 py-1 bg-red-50 dark:bg-red-950/60 text-red-600 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/60 rounded-lg text-xs font-bold transition-all"
                    >
                      Clear Selection
                    </button>
                  )}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder:text-slate-400 rounded-xl pl-10 pr-4 py-2.5 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                />
              </div>

              {/* Scrollable Checkbox List */}
              <div className="border border-slate-200 dark:border-slate-800 rounded-2xl max-h-64 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800 bg-slate-50/50 dark:bg-slate-800/30 p-1.5 scrollbar-hide">
                {filteredManualRoster.length === 0 ? (
                  <div className="p-4 text-center text-xs text-slate-400 dark:text-slate-500 font-medium">No residents match your search filter.</div>
                ) : (
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 rounded-xl flex items-center justify-between cursor-pointer transition-all ${
                          isSelected ? 'bg-blue-50 dark:bg-blue-950/40 border border-blue-200/80 dark:border-blue-800/80 shadow-xs' : 'hover:bg-slate-100/80 dark:hover:bg-slate-800/60'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <input
                            ...
                            checked={isSelected}
                            onChange={() => {}}
                            className="w-4 h-4 text-blue-600 rounded border-slate-300 dark:border-slate-600 focus:ring-blue-500 pointer-events-none"
                          />
                          <div className="flex flex-col">
                            <span className="font-bold text-slate-800 dark:text-white text-sm">{r.full_name}</span>
                            <span className="text-[11px] text-slate-400 dark:text-slate-400 font-medium">{r.email}</span>
                          </div>
                        </div>

                        {r.class_label && (
                          <span className="text-[11px] font-extrabold px-2.5 py-1 rounded-lg bg-slate-200/80 dark:bg-slate-700 text-slate-700 dark:text-slate-200">
                            {r.class_label}
                          </span>
                        )}
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={selectedYear}
                  onChange={(e) => setSelectedYear(Number(e.target.value))}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-2xl px-4 py-3.5 font-semibold outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none"
                >
                  {getAvailableAcademicYears().map(year => (
                    <option key={year} value={year}>{formatAcademicYear(year)}</option>
                  ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-2xl px-4 py-3.5 font-semibold outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder:text-slate-400 rounded-2xl px-4 py-3.5 font-medium outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-blue-700 transition-all disabled:opacity-50 shadow-xl shadow-blue-100 dark:shadow-none"
            >
